...
int numeroComb(int deg, int d, int *numComb);
...
extern void inizializza(VECTOR x, int n, type c); //inizializza VECTOR x al valore c
extern void inserisciComb(int n, int nCH, int degh, int indice, int* mComb, MATRIX xast, MATRIX x, int mt, int d, int deg); //assegna valori a x*
...
/**
 * La funzione calcola un vettore risltate a seguito della seguente formula:
 * for(i from 1 to len)
 * vector_dest[i] = vector_dest[i] + fact_mul * vector_src[i];
 * @param fact_mul, fattore moltiplicativo
 * @param vector_src, vettore che funge da operando da sorgente
 * @param vector_dest vettore che funge da operandoe da destinazione
 * @param len, lunghezza del vettore
 */
extern void calcolaVettore(type fact_mul, VECTOR vector_src, VECTOR vector_dest, int len);
extern type prodottoScalare(VECTOR stimaTheta, VECTOR dBatchJ, int len, type* indirizzo_ret);
 ...
 * cambioBase
 * ==========
 * riceve un numero i in base bp, e lo converte in base ba
 */
...
void combinazioni2(int* ris, int h, int d) {
	...
	for(i=0;!flag; i++, start++){
		...
		divi/=d; //avanzamento alla variabile pi� a sinistra
	    if(cur == d-1)// se l'ultimo numero � uguale a d-1 al prossimo start ci sar� un numero non ordinato
	        flag=1;
		ris[i*h +h-1]=cur;
		for(j=h-2; j>=0; j--){
		    cur = divi % d;
...
void convert_data(params* input){
    ...
    for( h=1; h<=deg; h++){
            nCH = numComb[h];
            inserisciComb(n, nCH, deg-h, indice, mComb, ret, x, mt*4, d*4, deg);
            indice += nCH;
    }
    input->t=t;
    input->xast=ret;
 ...
 * @param yBatch, sottoinsieme della matrice y che considera il batch, quindi indirizzo
 * al primo elemento del vettore blocco del vettore y che la dimensione del batch ed
 * il numero del blocco (i) che si sta considerando
...
/**
 * Calcola il valore della sommatoria
 *
 * @param stimaTheta theta precedente
 * @param Dbatch, sottoinsieme della matrice D in cui sono prese solo le righe che rientrano nell'batch
 * @param yBatch, sottoinsieme della matrice y che considera il batch
 * @param indexSommatoria , valore i + v - 1 a cui deve arrivare la sommatoria
