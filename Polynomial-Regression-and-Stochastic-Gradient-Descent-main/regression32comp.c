...
extern void inserisciComb(int deg, int nCH, int degh, int* mComb, MATRIX xast, MATRIX x); //assegna valori a x*
...
/**
 * La funzione calcola un vettore risultate a seguito della seguente formula:
 * for(i from 1 to len)
...
void convert_data(params* input){
    ...
    for( h=1; h<=deg; h++) {
        ...
        #pragma omp for
        for(oss=0; oss<n; oss++)
            inserisciComb(deg, numComb[h], deg-h, mComb, ret+indice[h-1]+oss*mt, x+oss*d);
        }
    }
    input->t=t;
    input->xast=ret;
...
void calcolaTheta_adagrad(MATRIX Dbatch, MATRIX G, int v, VECTOR yBatch, VECTOR stimaTheta, type eta, int t, int mt, VECTOR sommatoria) {
    ...
    calcolaVettore(mul, sommatoria, stimaTheta, mt);
}
...
/**
 * Calcola il valore della sommatoria dentro la funzione SGD
 * @param stimaTheta theta precedente
 * @param Dbatch, sottoinsieme della matrice D in cui sono prese solo le righe che rientrano nell'batch
...
/**
 * La funzione calcola il valore della sommatoria dentro funzione adagrad
...
void sommatoriaAdagrad(VECTOR stimaTheta, MATRIX Dbatch, type G[], VECTOR yBatch, int indexSommatoria, int t, int mt, VECTOR sommatoria) {
    ...
    for (int j = 0; j < indexSommatoria; j++) {
        type gjv;
        type ps;
        prodottoScalare(stimaTheta, (Dbatch + j * mt), mt, &ps);
        type delta = ps - yBatch[j];
        sommatoriaInterna(delta, (Dbatch + j * mt), (G + j * mt), mt, sommatoria);
    }
}
