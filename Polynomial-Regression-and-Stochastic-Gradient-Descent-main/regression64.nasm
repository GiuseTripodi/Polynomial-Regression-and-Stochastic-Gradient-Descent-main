...
calcolaVettore:
    ...
    ;usiamo un loop unrolling di 8 elementi, scorriamo quindi 32 elementi per volta
...
;extern void inserisciComb(int n=RDI, int nCH=RSI, int degh=RDX, int mt=RCX, int* mComb=R8, MATRIX xast+indice=R9, MATRIX x, int d, int deg);
...
iC_oss: DEC         RDI                             ;n--
        JZ          iC_fin                          ;n=0 salta
;       for(c=0; c<numComb[h]; c++) //c e' J, per ogni J appartenente a [d]^h
        MOV         RBX, R8                         ;resetto il puntatore a mComb
        XOR         R14, R14                        ;c=0
...
;        loop vectorization
        MOV         EAX, [RBX+R13*4]                ;EAX=mComb[c*deg+i] prendo un indice della combinazione
        VMOVSD      XMM2, [R15+RAX*8]               ;prendo x[indice della combinazione] e.g. x[2]
        LEA         R12, [RBX+R11*4]                ;i+deg => raggiungo l'elemento della riga successiva

        MOV         EAX, [R12+R13*4]                ;EAX=j[indice_comb*deg+i] prendo un indice della combinazione
        VPINSRQ     XMM2, XMM2, [R15+RAX*8], 00000001b    ;prendo x[indice della combinazione] e.g. x[2]
        LEA         R12, [R12+R11*4]                ;i+deg => raggiungo l'elemento della riga successiva

        MOV         EAX, [R12+R13*4]                ;EAX=j[indice_comb*deg+i] prendo un indice della combinazione
        VMOVSD      XMM1, [R15+RAX*8]               ;prendo x[indice della combinazione] e.g. x[2]
        LEA         R12, [R12+R11*4]                ;i+deg => raggiungo l'elemento della riga successiva

        MOV         EAX, [R12+R13*4]                ;EAX=j[indice_comb*deg+i] prendo un indice della combinazione
        VPINSRQ     XMM1, XMM1, [R15+RAX*8], 00000001b    ;prendo x[indice della combinazione] e.g. x[2]
...
iC_Fi:
        ;mi sono già spostato di 4 righe su mComb(lo faccio riga per riga)
        LEA         RBX, [R12+R11*4]                ;i+deg => raggiungo l'elemento della riga successiva
...
iC_RIi: CMP         R13, RDX                        ;i<deg-h
	    ...
	    MOV         EAX, [RBX+R13*4]                ;EAX=j[indice_comb*deg+i] prendo un indice della combinazione
        VMULSD      XMM0, [R15+RAX*8]               ;ac*=x[indice della combinazione]
        DEC         R13                             ;i--
        JMP         iC_RIi

iC_FRIi:
        LEA         RBX, [RBX+R11*4]                ;i+deg => raggiungo l'elemento della riga successiva
        VMOVSD      [R9+R14*8], XMM0                ;ret[indice]=XMMO[0]
        INC 		R14
        JMP         iC_RIc

iC_Fc:
        ADD		    R9, RCX			                ;xast+mt*8 , salto alla riga successiva di xast, RCX=mt*8
        ADD		    R15, R10            			;x+d*8 , salto alla riga successiva di x, R10=d*8
        JMP         iC_oss
