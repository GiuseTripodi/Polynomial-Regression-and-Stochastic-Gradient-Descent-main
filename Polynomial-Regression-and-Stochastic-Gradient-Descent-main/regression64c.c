...
extern void inserisciComb(int n, int nCH, int degh, int mt, int* mComb, MATRIX xastSomm, MATRIX x, int d, int deg); //assegna valori a x*
...
void convert_data(params* input){
    ...
    for( h=1; h<=deg; h++){
        nCH = numComb[h];
        inserisciComb(n, nCH, deg-h, mt, mComb, ret+indice, x, d, deg);
        indice += nCH;
    }
    input->t=t;
    input->xast=ret;
